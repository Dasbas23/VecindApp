...
- Filtrar servicios por categoría (pictograma ARASAAC) y rango de horas [Excluido de la MVP]
...
- Consultar el historial de transacciones con gráfico de barras [Excluido de la MVP]
- Escuchar la descripción de un servicio en voz alta (TTS)
- Recibir notificaciones locales de caducidad de servicios [Excluido de la MVP]
...
- Historial / Gráfico (BarChart MPAndroidChart) [Excluido de la MVP]

### Flujo de navegación
- **Inicio:** Splash → Login (selección de perfil)
- **Pantalla principal:** Escaparate del Barrio
  - FAB "+" → Publicar Servicios [¿Que es FAB?]
  - Tap en tarjeta → Detalle de Servicio → (si acepta) → Valorar Servicio
  - Menú inferior → Mi Perfil → Historial [Excluido de la MVP] / Gráfico [Excluido de la MVP]
- **Navegación:** Navigation Component con Bottom Navigation Bar [¿Servrirán los gestos?]
...
### Modelo de datos
Diagrama E-R con 4 entidades (cumple mínimo normativo de 3 tablas relacionadas): [Necesito más info sobre las abreviaturas]
...
barrio [ex]     titulo              id_vendedor_fk FK→U  id_valorador_fk FK→U
avatar_path[ex] descripcion         id_servicio_fk FK→S  id_valorado_fk FK→U
saldo_horas     categoria           horas_transferidas   pictogramas_json
intercamb..[ex?]pictograma_id       estado               comentario 
nivel           coste_horas         timestamp            timestamp
fecha_registro  estado
tipo_usuario    fecha_publicacion
                fecha_caducidad
```
[ex] -> [Excluido de la MVP]
**Relaciones:**
...
- Configurar `AppDatabase` con `Room.databaseBuilder()` y migrations [¿Que es migrations?]
- Implementar los Repositorios (interfaz en domain + implementación en data) [Quiero explicaciones sobre el significado de esta linea]
- Poblar la BD con datos de prueba mediante `RoomDatabase.Callback` para el desarrollo [Se pedirán a un LLM que genere datos y los inserto en la BBDD]
...
- Guardar el perfil activo en `SharedPreferences` (id del usuario en sesión) [Excluido de la MVP]
- Implementar `SessionManager` (singleton) para recuperar el usuario activo en cualquier ViewModel [Excluido de la MVP]
